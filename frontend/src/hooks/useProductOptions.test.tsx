...
describe('useProductOptions', () => {
  it('returns loading then the list of product options', async () => {
    const { result } = renderHook(() => useProductOptions(), { wrapper: createWrapper() });

    expect(result.current.isLoading).toBe(true);

    await waitFor(() => expect(result.current.isSuccess).toBe(true));

    expect(result.current.data).toEqual(productOptionsFixture);
  });
