...
This spec defines the React-based web application for Tacosoft restaurant management system. The frontend provides a multi-tenant, role-based interface for staff to authenticate, manage orders, menus, billing, and operations in real-time.

---

## Scope

This specification covers the `frontend/` React application. The first slice (`frontend/scaffold-auth-shell-dashboard`) establishes the project, auth, shell, and dashboard. Later slices add domain CRUD, WebSocket, and PWA scaffolding.
...
The frontend specification defines the following capabilities:
...
- `frontend-dashboard`: KPI cards and dashboard report consumption.
- `frontend-tables`: Table grid and status management.
- `frontend-menu`: Section/category/product/option CRUD operations.
- `frontend-orders`: Order creation, listing, detail view, and kitchen operations.
- `frontend-billing`: Invoice listing and payment processing.
- `frontend-cash`: Register management and reporting (X/Z reports).
- `frontend-reports`: Sales, product, financial, footfall, and staff planning analytics.
- `frontend-suppliers`: Supplier CRUD and search operations.
- `frontend-websocket`: Real-time updates via STOMP/SockJS connection.
- `frontend-pwa`: Progressive Web App support with offline capabilities.

---

## Removed Requirements

None. The frontend specification is a pure addition to the Tacosoft platform.
